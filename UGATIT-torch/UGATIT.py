...
class UGATIT(object) :
    ...
    def build_model(self):
        ...
        self.trainB = ImageFolder(os.path.join('dataset', self.dataset, 'trainB_sr'), train_transform)
        self.testA = ImageFolder(os.path.join('dataset', self.dataset, 'testA'), test_transform)
        self.testB = ImageFolder(os.path.join('dataset', self.dataset, 'testB_sr'), test_transform)
        self.trainA_loader = DataLoader(self.trainA, batch_size=self.batch_size, shuffle=True, num_workers=4, pin_memory=True)
        self.trainB_loader = DataLoader(self.trainB, batch_size=self.batch_size, shuffle=True, num_workers=4, pin_memory=True)
        self.testA_loader = DataLoader(self.testA, batch_size=1, shuffle=False, num_workers=4, pin_memory=True)
        self.testB_loader = DataLoader(self.testB, batch_size=1, shuffle=False, num_workers=4, pin_memory=True)
    ...
    def test(self):
        model_list = glob(os.path.join(self.dataset, 'model', '*.pt'))
        if not len(model_list) == 0:
            model_list.sort()
            iter = int(model_list[-1].split('_')[-1].split('.')[0])
            self.load(os.path.join(self.dataset, 'model'), iter)
            print(" [*] Load SUCCESS")
        else:
            print(" [*] Load FAILURE")
            return
